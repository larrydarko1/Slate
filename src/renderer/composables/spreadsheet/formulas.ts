...
export type SpreadsheetFormulas = {
...
export function createFormulas(state: SpreadsheetCoreState, deps: FormulasDeps): SpreadsheetFormulas {
    function buildCellReferenceString(targetTableId: string, col: number, row: number): string {
        ...
        if (targetInfo === null) return cellRef;

        const sourceInfo = deps.findTableGlobal(state.activeCell.value.tableId);
        const targetTableName = targetInfo.table.name;
        const quoteTable = (n: string): string => (n.match(/^[A-Za-z_]\w*$/) !== null ? n : `'${n}'`);

        if (sourceInfo !== null && sourceInfo.canvas.id === targetInfo.canvas.id) {
            return `${quoteTable(targetTableName)}::${cellRef}`;
        }

        const targetCanvasName = targetInfo.canvas.name;
        const quoteCanvas = (n: string): string => (n.match(/^[A-Za-z_]\w*$/) !== null ? n : `'${n}'`);
    ...
    function getFormulaHighlights(): { tableId: string; col: number; row: number; color: string }[] {
        let tokens: FormulaToken[];

        if (state.isEditing.value && state.editValue.value.startsWith('=')) {
            tokens = getFormulaTokens();
        } else if (!state.isEditing.value && state.activeCell.value !== null) {
            const cell = deps.findCell(
