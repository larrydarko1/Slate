...
// ── formatCellDisplay: numeric formatting per type ───────────────────────────

describe('formatCellDisplay numeric formatting', () => {
    it('returns empty string for null', () => {
        expect(formatCellDisplay(null, 'integer')).toBe('');
    });

    it('formats integer', () => {
        expect(formatCellDisplay(42, 'integer')).toBe('42');
        expect(formatCellDisplay(3.7, 'integer')).toBe('4');
    });

    it('formats float with default precision', () => {
        expect(formatCellDisplay(3.14, 'float')).toBe('3.14');
        expect(formatCellDisplay(5, 'float')).toBe('5.0');
    });

    it('formats float with custom decimal places', () => {
        expect(formatCellDisplay(3.14159, 'float', 2)).toBe('3.14');
    });

    it('formats currency_usd', () => {
        expect(formatCellDisplay(1234.5, 'currency_usd')).toBe('$1,234.50');
        expect(formatCellDisplay(-50, 'currency_usd')).toBe('-$50.00');
    });

    it('formats percent', () => {
        expect(formatCellDisplay(0.5, 'percent')).toBe('50%');
        expect(formatCellDisplay(0.125, 'percent')).toBe('12.5%');
    });

    it('formats boolean', () => {
        expect(formatCellDisplay(1, 'boolean')).toBe('TRUE');
        expect(formatCellDisplay(0, 'boolean')).toBe('FALSE');
    });
});
