...
const LENGTH_BASELINE = {
    ...
    // Chart CRUD plus the range-picking mode that arms it. The picker is a
    // separate concern that only lives here because it writes chart refs.
    'src/renderer/composables/spreadsheet/charts.ts': 435,
    // One line over, and it has a real seam: detection (detectType and the
    // currency parsers) against formatting and coercion.
    'src/renderer/composables/spreadsheet/engine/cellTypes.ts': 401,
};
