 ...
 *      malicious .slate file — an opened workbook is arbitrary attacker-supplied
 *      JSON — and script execution inside a window that can talk to the main
 *      process.
 ...
 *   3. NO REMOTE SUBRESOURCES. Slate's claim is that nothing leaves the device.
 *      A single remote `<script>`/`<link>` in the shell breaks that claim, at
 *      the one point where CSP is also the only guard.
 *   4. THE REDUCED-MOTION ESCAPE HATCH. 20 transition/animation declarations are
 *      spread across 7 SFCs and 3 stylesheets; one global override is what makes
 *      all of them honour `prefers-reduced-motion`. Stylelint can police how a rule is
...
if (cspMatch === null) {
    ...
    const REQUIRED_DIRECTIVES = [
        ...
        {
            what: 'frame-ancestors or frame-src',
            re: /frame-(ancestors|src)\s+[^;]+/,
            why: 'Slate embeds nothing, so the policy should say so out loud — an unset directive falls through to default-src, which would allow one.',
        },
    ];
    ...
    const BANNED = [
        {
            what: "'unsafe-eval'",
            re: /'unsafe-eval'/,
            why: 'The formula engine has its own parser, and SFCs are precompiled, so nothing here needs it. Its presence means something started evaluating strings.',
        },
        {
            what: "'unsafe-inline' in script-src",
            re: /script-src[^;]*'unsafe-inline'/,
            why: 'Turns every value an opened workbook can put on screen into script execution. (style-src may keep it — Vue :style bindings need it.)',
        },
        {
    ...
    for (const m of csp.matchAll(/\b(https?|wss?):\/\/([^\s;]+)/g)) {
        ...
        if (!/^(localhost|127\.0\.0\.1)(:\d+)?\/?$/.test(host)) {
            fail(
                ...
                'Nothing in Slate may reach the network. Only the dev-server HMR socket on localhost is permitted here.',
            );
        }
    }
}
...
if (reducedMotionBlocks.length === 0) {
    fail(
        ...
        'It is the only place the 20 transition/animation declarations across 7 SFCs and 3 stylesheets can be switched off at once.',
    );
