 ...
 *   4.  setWindowOpenHandler DENIES. Without it a link inside a .slate file
 *       opens a full Electron window on a remote origin, with its own
 ...
 *       Electron denies by default, but the dev build runs on http://localhost,
 *       where the defaults are laxer — so the policy is written down rather than
 *       inherited, and a handler that forgets its else-branch grants everything.
 *   8.  A CUSTOM PROTOCOL CHECKS ITS BOUNDARY. Slate registers none today —
 ...
 *       cell contains — `file://`, `smb://`, on Windows a `.lnk` — which is
 *       arbitrary local execution reached by clicking a cell that looks like a link.
 ...
 *       where its string came from, which no regex can decide — so the allowlist
 *       below is empty and any NEW binding fails until someone writes down why it
 *       is safe.
 *  13.  NO REGEXP BUILT FROM UNESCAPED INPUT. Table and canvas names reach
 *       buildNamePattern, which compiles them. Unescaped, a name containing `(`
 *       throws at runtime and a name containing `(.*)+` is a ReDoS.
...
const REQUIRED_PREFS = [
    ...
    },
    {
        ...
        why: 'Set true, a window rendering an untrusted .slate file gets `require` — and with it the filesystem, child_process and the network.',
    },
    {
        key: 'sandbox',
        re: /sandbox:\s*true/,
...
if (!/setWindowOpenHandler\(/.test(allMain)) {
    fail(
        ...
        'Without it a link inside a .slate file opens a real Electron window on a remote origin.',
    );
} else {
...
for (const [name, why] of [
    [
        ...
        'Handles a live request. Absent, the file:// build denies everything and the http://localhost dev build does not, so the two disagree — and the day one permission is granted, this handler is the entire policy.',
    ],
    [
        ...
        for (const interp of m[1].matchAll(/\$\{([^}]*)\}/g)) {
            ...
            if (!escapedHere) {
                ...
                fail(
                    ...
                    "A table name containing `(` throws at runtime; one containing `(a+)+` is a ReDoS that hangs the process. Escape it with `.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')` first.",
                );
            }
        }
    }
    ...
    for (const bad of ['setCertificateVerifyProc', 'ignoreCertificateErrors', "'certificate-error'"]) {
        if (code.includes(bad)) {
            ...
            fail(
                `${rel}:${line}`,
                `uses \`${bad}\``,
                'Slate makes no network requests, so there is no certificate to have an opinion about. Its appearance means something started talking to a server.',
            );
        }
    }
}
